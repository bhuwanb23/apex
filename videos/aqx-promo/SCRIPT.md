# SCRIPT — aqx-promo

**Voice:** Kokoro `am_michael` (offline — HeyGen not signed in)
**Voice settings:** clear · medium pace · premium product tone
**Voice direction:** Confident, calm, high-end — never shouty sports-announcer. Short cues with natural pauses between thoughts.

---

## Line 1 — Brand open (Frame 1)

**Time:** 0.0 – 7.0s
**Delivery:** Soft, assured. Land “Personalized.”

    This is AQX. Sports intelligence. Personalized.

## Line 2 — The promise (Frame 2)

**Time:** 7.0 – 16.0s
**Delivery:** Slight tension on “noise,” release on “act on.”

    Every game dumps noise — box scores, play-by-play, gut feel. AQX turns it into a brief you can act on.

## Line 3 — Meet AQX (Frame 3)

**Time:** 16.0 – 24.0s
**Delivery:** Warm roll-call; pause between roles.

    Built for trainers. Coaches. Analysts. And fans who want the plain-English version.

## Line 4 — Injury Risk (Frame 4)

**Time:** 24.0 – 34.0s
**Delivery:** Protective, precise.

    Know who's at risk before they get hurt — traffic-light scores, workload spikes, and alerts you can share.

## Line 5 — Decisions (Frame 5)

**Time:** 34.0 – 44.0s
**Delivery:** Crisp; emphasize “pure logic.”

    Grade every coaching decision on pure logic — expected value, not just the final score.

## Line 6 — Momentum (Frame 6)

**Time:** 44.0 – 52.0s
**Delivery:** Curious, then resolved on “real.”

    Scrub the game. Watch the run. Find out if momentum is real — or just a story.

## Line 7 — Close (Frame 7)

**Time:** 52.0 – 60.0s
**Delivery:** Warm lockup; final period lands.

    AQX Sports Intelligence. Clear insights. Personalized for how you work.
